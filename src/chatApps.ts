import { ChatApp } from "./types";

export const chatAppList: ChatApp[] = [
  {
    name: "chatgpt",
    url: "chatgpt.com",
    icon_svg_data_uri:
    ...
    paste_function: (text: string, send: boolean) => {
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs[0]?.id) {
          chrome.scripting.executeScript({
            target: { tabId: tabs[0].id },
            func: (textToPaste: string, send: boolean) => {
              // Paste text
              const textArea = document.getElementById("prompt-textarea");
              if (!textArea) {
                console.error("Failed to paste prompt. Text area not found");
                return;
              }
              textArea.innerHTML = "<p>" + textToPaste + "</p>";
              const event = new Event("input", { bubbles: true });
              textArea.dispatchEvent(event);

              if (send) {
                // Wait for send button to appear
                let retriesCounter = 10;
                const intervalId = setInterval(() => {
                  const sendBtn = document.querySelector(
                    '#composer-background [data-testid="send-button"]',
                  ...
                  if (retriesCounter <= 0) {
                    clearInterval(intervalId);
                    console.log(
                      "Failed to paste prompt. Send button not found",
                    );
                  }
                }, 100);
              }
